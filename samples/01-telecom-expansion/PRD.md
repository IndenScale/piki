...
- 按机柜逐个输出 ✅
- 输出格式：SVG（可嵌入文档）+ PNG（可打印） ❌ 当前为纯文本格式
- 按机柜过滤：`piki generate rack-face-panel --rack RACK-A01` ❌
- 颜色编码：不同状态不同底色 ❌

**当前状态：** ⚠️ 已实现文本版面板图（`rack-face-panel` generator），
SVG/PNG 格式和按机柜过滤待后续阶段实现。
...
| 面板图缺少 SVG/PNG 输出 | 打印和嵌入文档不便 | 阶段 2 实现 SVG 渲染                          |
...
- [ ] 站点工程师从"打开 Excel 画面板图"改为"跑 `piki generate rack-face-panel`"
