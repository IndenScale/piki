...
- 支持 `--output` 指定输出路径 ✅
- 字段顺序和表头与施工队/采购习惯一致 ⚠️ 需对照实际模板

**当前状态：** ✅ 已实现基础版。telecom 插件内置 4 个生成器：

| 生成器 ID | 命令 | 功能 |
|---|---|---|
| `bom-csv` | `piki generate bom-csv` | 设备清单 CSV（7 列） |
| `rack-face-panel` | `piki generate rack-face-panel` | 机柜 U 位面板图（文本格式） |
| `rack-face-panel-svg` | `piki generate rack-face-panel-svg` | 机柜 U 位面板图（SVG 格式，可嵌入文档） |
...
- 图中标注：设备 ID、U 位范围、TDP ✅
- 按机柜逐个输出 ✅
- 输出格式：SVG（可嵌入文档）✅ / PNG（可打印）❌
- 按机柜过滤：`piki generate rack-face-panel --rack RACK-A01` ❌
- 颜色编码：不同状态不同底色 ❌

**当前状态：** ⚠️ 已实现文本版 + SVG 版面板图（`rack-face-panel` / `rack-face-panel-svg`），
PNG 格式、按机柜过滤和状态颜色编码待后续阶段实现。
...
**当前状态：** ✅ 已实现基础版。telecom 插件新增 `PortFamily`（设备端口）和 `PortConnectionFamily`（端口到端口连接），支持端口占用检查、连接端点检查、端口类型兼容性检查、线缆类型匹配检查，以及 `piki generate port-map` 端口分配表。
...
- `piki generate cable-schedule`：线缆排期表
- `piki generate cable-labels`：线缆打印标签 PDF
- `piki generate port-diagram`：端口互连图（SVG/PNG）
...
| US-5     | 面板图 / U 位正视图                    | ⚠️ 基础版 | 文本+SVG 已可用，PNG/过滤/颜色待实现 |
| US-6     | 线缆打印标签导出                       | ❌ 缺口   | 依赖 PortFamily+ConnectionFamily |
| US-7     | 端口图 / 互连接线图                    | ⚠️ 基础版 | PortFamily+ConnectionFamily 已就绪，端口图待实现 |
...
| 规则 ID            | 名称                 | 依赖                 | 说明                          |
| ------------------ | -------------------- | -------------------- | ----------------------------- |
| TELECOM-PORT-001   | 端口占用冲突检查     | `PortFamily`         | 同一设备内端口名不能重复      |
| TELECOM-PORT-002   | 端口所属设备存在性检查 | `PortFamily`         | device_id 引用必须存在        |
| TELECOM-CONN-001   | 连接端点存在性检查   | `PortConnectionFamily` | from_port / to_port 必须存在 |
| TELECOM-CONN-002   | 连接端口类型兼容性检查 | `PortConnectionFamily` | 两端 port_type 必须兼容      |
| TELECOM-CONN-003   | 连接线缆类型匹配检查 | `PortConnectionFamily` | cable_type 必须与端口类型匹配 |
