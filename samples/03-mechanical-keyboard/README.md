...
piki 帮你：**在下单打样前，自动发现 stem、针脚、开孔、电流、续航、天线、环境适应性、制造工艺等跨组件兼容性问题。**
...
│   ├── assembly/SUB-ALPHA.yaml    # 字母区子装配体
│   ├── cases/CASE-01.yaml
...
│   ├── operating_environments/INDOOR-USE.yaml  # 使用环境
│   └── materials/PBT.yaml         # 材料耐候性
...
| **环境适应性** | KB-ENV-001：户外使用 ABS 键帽报警；ENV-MAT-*：温度/IP/UV/盐雾 |
| **制造工艺约束** | DFX-002/003/004/005：CNC 尺寸、壁厚、注塑拔模角、表面处理 |
| **BOM/规格生成** | `keyboard-bom`、`keyboard-spec` Generator |
| **电气网络（Net）** | `instances/nets/`：多节点网络表达 VBUS/GND/矩阵行/列 |
| **跨插件协作** | `keyboard` + `consumer-electronics` + `environments` + `manufacturing` 插件共用类型与规则 |
...
| **装配层级** | `AssemblyFamily` + `SUB-ALPHA` 子装配体，支持 `mated_descendants`/`mated_ancestors` |
| **KiCad 导入** | `consumer-electronics/importer.py`：netlist / BOM / pick-and-place POC |
...
```yaml
# 把键帽材料改成 ABS 并关联户外环境，触发 ENV-MAT-002/003
# instances/materials/ABS.yaml
uv_resistant: false
# instances/operating_environments/INDOOR-USE.yaml
...
- [piki-environments 插件源码](../../src/piki/extensions/environments/plugin.py)
- [piki-manufacturing 插件源码](../../src/piki/extensions/manufacturing/plugin.py)
