...
> **piki** 是一个 Software-Defined Hardware (SDH) Framework。
> 它采用 Text-Native 声明式建模：工程师用 YAML 声明设计意图，Python 规则自动校验合理性，Git 追踪每一次变更。
...
│   ├── tests/                  #   ADL 自身测试
│   ├── docs/                   #   ADL 技术规范与架构决策（ADL 真相源）
│   └── pyproject.toml          #   ADL 包配置
├── src/piki/                   # piki 编排框架（可通过 pip 安装）
...
| `docs/adr/`                          | piki 框架层架构决策记录                |
| `adl/docs/adr/`                      | ADL 实现层架构决策记录                 |
| `docs/rfcs/`                         | 功能需求提案                           |
| `studio/ARCHITECTURE.md`             | Studio（TypeScript）架构               |
| `ROADMAP.md`                         | piki 当前阶段与未来计划                |
