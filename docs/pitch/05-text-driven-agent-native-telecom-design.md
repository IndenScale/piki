...
piki 是 **Agent-Native** 设计的。文本 YAML 是唯一真相源，Agent 可以直接读写；CLI 是第一接口，`piki check` / `piki generate` 全部通过命令行完成，输出结构化诊断（LSP 兼容格式，支持 VS Code / Vim 集成）；GUI（Piki Studio 3D 预览）只是给人看的消费层，Agent 不碰。
...
- [软件定义硬件（SDH）：大模型落地工业的必由之路](01-why-sdh.md) — 形式化程度光谱与 SDH 世界观
- [Agent Native 工业软件：为 Agent 时代重新设计底层](02-agent-native.md) — ANIS 五项原则
