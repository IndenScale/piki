...
**生成器不修改设计，只消费设计。** 这与 Checker 的规则引擎形成对称：
...
| ADR-004（空间可视化）   | 互补 | Studio 是交互式沟通媒介，Generator 是标准化交付物     |
| ADR-006（CAD 资产引用） | 互补 | Generator 可引用 `assets.mesh` 渲染带厂商几何的面板图 |
