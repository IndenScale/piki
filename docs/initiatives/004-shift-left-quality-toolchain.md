...
| 工具 | 层级 | 能力 |
|---|---|---|
| `adl lint` | L0-L1b | YAML/TOML 语法、schema 校验、命名风格 |
| `adl validate` | L2 | 引用完整性、Mate 自洽、Layout 合法性、Catalog 解析 |
| `piki check` | L3 + L4a + L5a | 领域规则、低成本几何/物理快速检查、生成物检查 |
| `piki verify geom` | L4b | CAD 内核精确几何验证（干涉、间隙、包容、可达性） |
| `piki verify phys` | L5b | 外部 CAE/仿真求解器验证（CFD、FEA 等） |
| `piki signoff` | L6 | 汇总所有证据，支持专家/AI 综合签核 |
| LSP server | L0-L5a | 编辑器实时诊断（低成本检查） |
...
| 层级 | 负责工具 | 说明 |
|---|---|---|
| L0-L1b | `adl lint` | 语法、Schema 与风格/约定检查 |
| L2 | `adl validate` | 引用、Mate、Layout、Catalog 自洽 |
| L3 + L4a + L5a | `piki check` | 领域规则、快速几何/物理检查、生成物检查 |
| L4b | `piki verify geom` | CAD 内核精确几何验证 |
| L5b | `piki verify phys` | 外部仿真/求解器验证 |
| L6 | `piki signoff` | 汇总所有证据，专家/AI 综合签核 |

完整层级定义、诊断格式与实现 rationale 见 [ADR-003](../adr/engine-and-plugins/003-quality-checks-and-diagnostics.md)。

本动议的重点是把 L0-L5a 工具化、产品化、可扩展化；同时让 L4b/L5b 成为可被 HDA 编排的外部验证后端，让 L6 签核层能够汇总所有证据。
...
DRC 在 EDA 领域已经是一个成熟概念（间距、宽度、天线效应等）。在 piki 中，DRC 应被扩展为跨领域的「设计规则检查」，并区分两层：

- **L3/L4a/L5a 快速 DRC**：默认在 `piki check` 中执行，用经验公式或轻量几何/物理规则在秒级内拦截常见错误。
- **L4b/L5b 精确 DRC**：通过 `piki verify geom/phys` 调用 CAD/CAE 后端，在 milestone 或签核时执行。
...
### 阶段二：静态分析与精确验证（6-12 个月）

- 增强 `adl validate` 的静态分析能力（未引用、循环依赖、冗余等）；
- 在 `piki check` 中完善 L3 + L4a + L5a 的快速规则库；
- 实现 `piki verify geom` 命令，对接 CAD 内核执行精确几何验证；
- 实现 `piki verify phys` 命令原型，对接 CAE 求解器；
- 与 [EPM](./002-epm-engineering-package-manager.md) 集成，读取几何/网格资产。

### 阶段三：生态与平台（12-24 个月）

- 实现 `piki signoff` 签核层，汇总 L0-L5b 证据并支持专家/AI 评审；
- 开放规则市场，允许厂商和社区贡献规则包；
- 规则包本身通过 EPM 分发；
