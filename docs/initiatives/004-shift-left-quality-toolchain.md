...
[ADR-003](../adr/engine-and-plugins/003-quality-checks-and-diagnostics.md) 已经定义了 L0-L6 分层检查：

| 层级 | 名称 | 负责工具 | 说明 |
|---|---|---|---|
| L0 | 语法 | `adl lint` | YAML/TOML 是否可解析 |
| L1 | Schema | `adl lint` | 字段类型、必填项、枚举值 |
| L2 | 结构自洽 | `adl validate` | 引用、Mate、Layout、Catalog |
| L3 | 领域规则 | `piki check` | 插件规则：功率、容量、兼容性等 |
| L4 | 生成物 | `piki check` | BOM、报告、3D 场景生成正确 |
| L5 | 仿真/外部验证 | `piki drc --deep` | 调用外部求解器 |
| L6 | 专家审查 | 人工 | 不可自动化的设计判断 |
