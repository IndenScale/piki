# Initiative-001: SD-HWE-Bench —— 软件定义硬件的能力竞技场

- **状态：** 倡议 / 讨论中
- **日期：** 2026-06-13
- **作者：** piki 核心团队与社区
- **相关文档：** [软件定义硬件（SDH）](../pitch/01-why-sdh.md)、[Engineering RLVR](../pitch/04-engineering-rlvr.md)、[ADR-003: 多级质量检查](../adr/engine-and-plugins/003-quality-checks-and-diagnostics.md)、[piki 的生态位](../concepts/05-ecosystem-positioning.md)
...
SD-HWE-Bench 不是 piki 主项目的功能，而是由 piki 社区发起并维护的**独立倡议**。piki 提供声明式建模语言、规则引擎和评分基础设施，但 benchmark 本身属于更广泛的 SDH 生态。
...
这正是 SDH 要解决的问题。如果设计意图不能以结构化文本存在、不能被规则自动校验，工程 Agent 就无法获得可验证的奖励信号，RLVR 飞轮就无法建立。关于 RLVR 在工程领域的适用性，详见 [Engineering RLVR](../pitch/04-engineering-rlvr.md)。
...
| CAD/BIM 交互操作 | SDH 主张 GUI 不应是唯一入口 | 不鼓励以 VLM 点击 GUI 的方式参赛 |
...
**当前倾向：SD-HWE-Bench**。它强调这不是传统硬件设计 benchmark，而是「软件定义」范式下的工程能力评测，与 piki 推动的 SDH 叙事一致。

---

## 5. 与现有 benchmark 的差异化

近期已有大量工程 AI benchmark 涌现。它们在测量对象上可分为多个层级：知识问答（如 TeleQnA）、文档理解（如 AECV-Bench）、文档协调 Agent（如 AEC-Bench）、代码/网表生成（如 VerilogEval、ChipBench）、约束生成与优化（如 EngDesign、Frontier-Eng），以及最接近 SD-HWE 范式的结构化设计意图 + 规则校验（如 AMS-IO-Bench）。更完整的梳理见 [工程领域 AI Benchmark 形态与缺口调研](../pitch/06-engineering-ai-benchmark-landscape.md)。
...
这与 SWE-Bench「修改代码并通过测试」的范式更接近。关于 SDH 与软件工程范式的对比，详见 [软件定义硬件（SDH）](../pitch/01-why-sdh.md)。
...
- 评分以 `piki check` L0-L5a 通过率为核心指标；
...
- 私有测试集防止过拟合；
- 引入 L4b / L5b 精确几何/物理仿真和 L6 专家/AI 签核作为可选扩展；
- 与学术会议/工业会议合作发布；
- 孵化独立组织或基金会治理。
...
| 仿真工具链依赖 | 部分任务难以自动化 | L4b / L5b / L6 作为可选扩展，不强求 |
...
- [软件定义硬件（SDH）](../pitch/01-why-sdh.md)
- [Engineering RLVR：可验证奖励如何驱动工程 AI](../pitch/04-engineering-rlvr.md) — SD-HWE-Bench 背后的 RLVR 范式论述
- [工程领域 AI Benchmark 形态与缺口调研](../pitch/06-engineering-ai-benchmark-landscape.md) — 现有 benchmark 与 SD-HWE-Bench 的差异化分析
