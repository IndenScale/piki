...
在 piki 的检查分层（[ADR-003: 多级质量检查体系](../adr/engine-and-plugins/003-quality-checks-and-diagnostics.md)）中，这对应 **L4b 精确几何检查**、**L5b 物理仿真** 和 **L6 签核与综合评估**。
...
在 piki 的检查分层中，这对应 **L3 跨记录业务规则**、**L4a 快速几何检查** 和 **L5a 快速物理规则**。
...
在 piki 的检查分层中，这对应 **L0 格式合法性**、**L1a Schema 校验** 和 **L2 单记录完整性**。
...
L6: 签核与综合评估 ───────────────────────┐
L5b: 物理仿真 ────────────────────────────┤  第一阶段：发现未知
L4b: 精确几何检查 ────────────────────────┤  （成本高，事后/签核时）
                                          │
L3: 跨记录业务规则 ───────────────────────┐
L4a: 快速几何检查 ────────────────────────┤  第二阶段：编码已知
L5a: 快速物理规则 ────────────────────────┤  （成本中等，check 时）
                                          │
L2: 单记录完整性 ─────────────────────────┐
L1b: 风格与约定检查 ──────────────────────┤  第三阶段：结构声明
L1a: Schema 校验 ─────────────────────────┤  （成本极低，加载时）
L0: 文件格式合法性 ───────────────────────┘
```
